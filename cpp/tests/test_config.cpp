...
TEST_CASE("ClientConfig has correct defaults", "[config]") {
    ClientConfig config;
    CHECK(config.base_url == "https://api.ensoul.ai");
...
TEST_CASE("api_url strips trailing slash and appends /v1", "[config]") {
    ClientConfig config;
    config.base_url = "https://api.ensoul.ai/";
    CHECK(config.api_url() == "https://api.ensoul.ai/v1");
}
...
TEST_CASE("Constants have expected values", "[config]") {
    CHECK(std::string(DEFAULT_BASE_URL) == "https://api.ensoul.ai");
